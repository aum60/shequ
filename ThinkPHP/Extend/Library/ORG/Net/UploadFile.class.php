...
class UploadFile {//类定义开始
     ...
     * @return string
     */
    private function save($file, $limit=array()) {
        ...
        if( in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png','swf'))) {
            ...
            //检测图片尺寸
//            if(!empty($limit)){
//                $this->error = $info[0] < $limit['width'] ? '图片尺寸应为'.$limit['width'].' * '.$limit['height'] : 'ok';
//                return false;
//            }
        }
     ...
     * @return string
     */
    public function upload($savePath ='', $limit = array()) {
        //如果不指定保存文件名，则由系统默认
        if(empty($savePath))
        ...
        foreach($files as $key => $file) {
            ...
            if(!empty($file['name'])) {
                ...
                //保存上传文件
                if(!$this->save($file,$limit)) return false;
