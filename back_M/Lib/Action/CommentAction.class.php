...
//    protected $upload_dir = './Public/Uploads/cover/';
    ...
    public function __construct() {
        ...
        if($ina_from){setcookie('ina_from', $ina_from, time() + 86400);}
        //消息数量显示
        $this->hfnum();
    }
   ...
   public function keyword() {
        $cc = M('Cat');
        $res = $cc->field('id,name')->select();
       // echo"<pre>";print_r($res);die;
        $confarr=array(1,4,999);
        foreach ($res as $key => $value) {
            if (in_array($value['id'], $confarr)) {
                $cat[]=$value;
            }
        }
        $this->assign('cat', $cat);
    ...
    public function updateImage($picname, $path, $prix = "s_", $w) {
        ...
        $width = exec("identify " . $path . "/" . $picname . " | cut -d ' ' -f 3 | cut -d 'x' -f 1");
        $height = exec("identify " . $path . "/" . $picname . " | cut -d ' ' -f 3 | cut -d 'x' -f 2");
        //获取原来图片的长宽比例
...
//        echo "convert " . $path . "/" . $picname . " -resize " . $w . "x " . $path . "/" . $prix . $picname;
//        die;
...
//                if ($_POST['type'] == 2) {
//                    //判断裁剪尺寸
//                    if (intval($_POST['w']) < 750 || intval($_POST['h']) < 500) {
//                        echo 1000;
//                        die;
//                    }
//                }
...
//                die;
                $pn = explode('/', $_POST['pn']);
                echo $this->cut_pic(end($pn), $this->upload_dir, $new_x, $new_y, $new_w, $new_h, "x_", $_POST['type']);
            }
        }
    }

    //根据点位坐标裁剪图片     (图片名称,图片路径,x坐标,y坐标,长度,宽度,前缀)
    public function cut_pic($picname, $path, $x, $y, $width, $height, $prix = "x_", $type) {
        //去除后面多余的"/"
        $path = rtrim($path, "/");
        //获取图片文件的属性信息
        if ($type == 1) {
...
//        }
        //执行裁剪
        exec("wget -O /tmp/" . $picname . " " . $ftp_yuancheng . $picname);
//        echo "wget -O /tmp/" . $picname . " " . $ftp_yuancheng . $picname;
//        die;
        //imagecopyresampled($newim, $srcim, 0, 0, $x, $y, $width, $height, $width, $height);
//        echo "convert " . "/tmp/" . $ftp_yuancheng . $picname . " -crop " . $width . "x" . $height . "+" . $x . "+" . $y . " " . $path . "/" . $prix . $picname;
//        die;
        exec("convert " . "/tmp/" . $picname . " -crop " . $width . "x" . $height . "+" . $x . "+" . $y . " " . $path . "/" . $prix . $picname);
        //保存图片
//        switch ($info[2]) {
//            case 1: //gif格式
//                imagegif($newim, $path . "/" . $prix . $picname); //保存
...
//        imageDestroy($srcim);
        ...
        //本地的的文件路径
        $aSource_file[] = $path . '/' . $prix . $picname;
//        echo $ftp_path . '/' . $prix . $picname;
//        echo $path . '/' . $prix . $picname;die;
        $this->ftp_copy_files($aDestination_file, $aSource_file, 0, FTP_BINARY);
        //删除裁剪前的图片
        unlink($path . "/" . $picname);
        unlink($path . '/' . $prix . $picname);
        unlink("/tmp/" . $picname);
//        $file[] = $ftp_path . '/' . $prix . $picname;
//        $this->ftp_del_files($file, 0);
    ...
    public function mynews() {
        if ($_SESSION[C('USER_AUTH_KEY')]['id']) {
         ...
         //echo"<pre>"; print_r($news_data);die;
            // echo M('NewsManagement')->getLastSql();die;
          if ($news_data) {
              foreach ($news_data as $key => &$v) {
                if ($v['act_userid']) {
                    $getname=D('Users')->get_user_info($v['act_userid']);
                    //print_r($getname);die;
                    $a="<a href='/index.php/Ucenter/uspace/uid/".$getname['id']."'>".$getname['username']."</a>";
                    //echo $a;die;
                }
    ...
    public function is_reade_all() {
        if ($_SESSION[C('USER_AUTH_KEY')]['id']) {
            $id = intval($_POST['id']);
            if ($id) {
                $sql = "update bbs_news_management set status=1 where userid=" . $_SESSION[C('USER_AUTH_KEY')]['id'] . ' and id=' . $id;
            } else {
                $sql = "update bbs_news_management set status=1 where userid=" . $_SESSION[C('USER_AUTH_KEY')]['id'];
            }
            $res = M('', 'bbs_', 'DB_CONFIG2')->query($sql);
            if (empty($res)) {
                echo 1;
