...
class CommAction extends CommonAction {
    ...
    public function _filter(&$map) {
        ...
        //
        //判断是否存在搜索条件
        if (!empty($_REQUEST['name'])) {//用户名搜索
            //$list1 = M("Users")->field("id")->where(array("name" => array("like", "%".$_REQUEST['name']."%")))->select();
            $list1=D('Users')->get_user_by_where("name like '%".$_REQUEST['name']."%'");
            $uid = array();
            foreach ($list1 as $v1) {
                $uid[] = $v1['id'];
            }
        ...
        if (!empty($_REQUEST['title'])) {//帖子名搜索
            $list2 = D("Message")->field("id")->where(array("title" => array("like", "%" . trim($_REQUEST['title']) . "%")))->select();
            $lid = array();
            foreach ($list2 as $v2) {
                $lid[] = $v2['id'];
            }
    ...
    public function _tigger_list(&$list) {
        ...
        //遍历之前查询出的数据
        foreach ($list as &$v) {
            //$ob1 = $mod1->field("name")->find($v['uid']);
            $ob1=D('Users')->get_user_info($v['uid']);
            $v['name'] = $ob1['name'];
