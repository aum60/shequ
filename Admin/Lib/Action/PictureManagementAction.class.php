...
//帖子信息控制Action类
class PictureManagementAction extends CommonAction {
    ...
    public function edit() {
        ...
        $vo = $model->where('id=' . $id)->find();
        $tmp = $this->_SERVER_LIST;
        $vo['picture_path'] = $this->ftp_img_path . $vo['picture'];
        $this->assign('vo', $vo);
        $session_id = session_id();
        $this->assign('session_id', $session_id);
        $time = time();
    ...
    protected function _list($model, $map = array(), $sortBy = '', $asc = false) {
        ...
        if ($list) {
            foreach ($list as &$v) {
                $v['picture'] = $this->ftp_img_path . $v['picture'];
                if ($v['status'] == 1) {
                    $v['status'] = '否';
                } else {
                    $v['status'] = '是';
    ...
    public function data_add() {
        //var_dump($_POST);die;
        if ($_POST) {
            $model = D("PictureManagement");
            $_POST['picture'] = $_POST['showpicval'];
            $_POST['addtime'] = time();
