...
//帖子信息控制Action类
class SysNewsManagementAction extends CommonAction {

    //添加搜索方法
    public function _filter(&$map) {
        //判断是否有搜索条件
        if ($_REQUEST['username']) {
            $list = M("Users")->field('id')->where("username like '%" . $_REQUEST['username'] . "%'")->select();
            $uid = array();
            foreach ($list as $vo) {
                $uid[] = $vo['id'];
            }
    ...
    public function all_examine() {
        if ($_POST) {
            $id_str = substr($_POST['str'], 0, -1);
            $sql = 'update bbs_message set status=1 where id in(' . $id_str . ')';
            D()->query($sql);
            echo 2;
        } else {
            echo 1;
    ...
    public function cut_pic_size() {
        if ($_POST) {
            if ($_POST['pn'] != '' && $_POST['x'] != '' && $_POST['y'] != '' && $_POST['w'] != '' && $_POST['h'] != '') {
                ...
                echo $this->cut_pic($_POST['pn'], $this->upload_dir, $_POST['x'], $_POST['y'], $_POST['w'], $_POST['h']);
            }
        }
    }
    ...
    public function cut_pic($picname, $path, $x, $y, $width, $height, $prix = "x_") {
        ...
        $info = getimagesize('http://img.news18a.com/community/cover/' . $picname);
        //新图片源
        $newim = imagecreateTrueColor($width, $height);
        //根据原图片类型来创建图片源
        switch ($info[2]) {
            case 1: //gif格式
                $srcim = imageCreateFromgif('http://img.news18a.com/community/cover/' . $picname);
                break;
            case 2: //jpeg格式
                $srcim = imageCreateFromjpeg('http://img.news18a.com/community/cover/' . $picname);
                break;
            case 3: //png格式
                $srcim = imageCreateFrompng('http://img.news18a.com/community/cover/' . $picname);
                break;
            case 6: //bmp格式
                $srcim = imageCreateFrompng('http://img.news18a.com/community/cover/' . $picname);
                break;
...
//        $this->ftp_del_files($file, 0);
    ...
    public function delete() {
        ...
        if ($_GET) {
            $id = intval($_GET['id']);
            $model = M("SysNewsManagement");
            $return = $model->where('id='.$id)->delete();
            $model2 =M("NewsManagement");
            $return2 = $model2->where('systemid='.$id)->delete();
            if ($return !== false && $return2 !== false) {
                $this->success(L('删除成功'));
            } else {
    ...
    public function data_add() {
        ...
        if ($_POST) {
            ...
            $model = D("SysNewsManagement");
            $return = $model->add($_POST);
            //查询所有的用户
            //$user_info = M('Users')->select();
            $user_info=D('Users')->select_all_user();
            $time = time();
    ...
    public function data_edit() {
        ...
        if ($_POST) {
            ...
            foreach ($user_info as $v) {
                ...
                M('NewsManagement')->where('systemid=' . $_POST['id'])->save($array);
            }
            if ($return) {
                $this->success(L('新增成功'));
            } else {
