...
class UsersAction extends CommonAction {
    ...
    public function _filter(&$map) {
        ...
        if ($_REQUEST['username']) {
            ...
            $list=D('Users')->get_user_by_where("name like '%".$_REQUEST['username']."%'");
            $uid = array();
            foreach ($list as $vo) {
                $uid[] = $vo['id'];
            }
    ...
    protected function _list($model, $map = array(), $sortBy = '', $asc = false) {
        ...
        $count = D('Users')->get_data_count($map);
        //echo 444;die;
        ...
        //var_dump($map);
        $list = D('Users')->get_user_by_where_limit($map,$p->firstRow . ',' . $p->listRows);
        if ($list) {
            foreach ($list as &$v) {
                $v['picture_path'] = $v['photo'];
                $v['picture'] = $v['photo'];
            }
        }
    ...
    public function all_gag() {
        if ($_POST) {
            ...
            $sql = 'update cms_app_user set is_gag=1 where id in(' . $id_str . ')';
            $result = D()->query($sql);
            echo 2;
        } else {
            echo 1;
    ...
    public function re_all_gag() {
        if ($_POST) {
            $id_str = substr($_POST['str'], 0, -1);
            $sql = 'update cms_app_user set is_gag=0 where id in(' . $id_str . ')';
            $result = D()->query($sql);
            echo 2;
        } else {
            echo 1;
    ...
    public function all_examine() {
        if ($_POST) {
            $id_str = substr($_POST['str'], 0, -1);
            $sql = 'update cms_app_user set picture="" where id in(' . $id_str . ')';
            D()->query($sql);
            echo 2;
        } else {
            echo 1;
