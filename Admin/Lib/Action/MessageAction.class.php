...
//            $order = !empty($sortBy) ? $sortBy : $model->getPk();
            $order = !empty($sortBy) ? $sortBy : 'addtime';
        }
...
//        echo $model->getLastSql();
        if ($list) {
            foreach ($list as &$v) {
                $v['cover2_path'] = $this->ftp_img_path . $v['cover2'];
                //浏览次数

                $v['scan'] = M('ViewRecord')->where('mid=' . $v['id'])->count();
                //【评论次数
                $v['plnum'] = M('Comm')->where('mid=' . $v['id'])->count();
                //查询分类
                $v['pid'] = substr($v['pid'], 1, -1);
                if ($v['pid']) {
                    $cat = M('Cat')->where('id in(' . $v['pid'] . ')')->select();
                    //echo M('Cat')->getLastSql();
                    $str = '';
                    foreach ($cat as $cv) {
                        $str .= $cv['name'] . ',';
                    }
