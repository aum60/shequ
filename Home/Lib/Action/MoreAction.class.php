...
class MoreAction extends CommentAction {
    ...
    public function index() {
        ...
        //加载更多
        $list2 = M('Message')->where('status = 1')->limit($go . "," . 20)->order('addtime desc')->select();
        foreach ($list2 as &$vo) {
...
//                $class_info = M('Cat')->where('id=' . $vo['pid'])->find();
                ...
                $vo['scan'] = $vie;
                $this->assign('vo', $vo);
                $htmlData[] = $this->fetch('More:boutique');
            }
        }
...
//                $aSource_file[] = $path . '/' . $prix . $picname;
        ...
        for($i=0; $i<100; ++$i){
        $datas = M('Message')->where('status = 1')->limit($n . "," . $m)->order('addtime desc')->select();
            if($datas){
                foreach($datas as $k=>$vo){
                    list($date,$name) = explode('/',$vo['cover2']);
                    $old_pic = $this->ftp_img_path . $vo['cover2'];
                    $new_pic = '/tmp/'.$name;
                    $res_pic = '/pic/'.$name;
//                    $res_pic = '/pic/'.$date.'/'.$name;
                    Http::curlDownload($old_pic,$new_pic);
//                    $this->image->open($new_pic);
//                    $this->image->thumb(600,300,3)->save($res_pic);
                    //将图片裁剪为指定比例并保存
                    $this->image->thumb(600,300,3)->save($res_pic);
...
//                    $this->image->thumb(600,300,3)->save($res_pic);
        ...
        //将图片裁剪为指定比例并保存
        $datas = File::get_dirs('/tmp/');
        foreach($datas['file'] as $k => $v){
            $this->image->open('/tmp/'.$v);
