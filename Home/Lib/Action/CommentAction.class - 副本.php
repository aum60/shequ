...
//        }
    ...
    public function updateImage($picname, $path, $prix = "s_", $w)
    {
        ...
        $image->open($picture);
        $image->thumb(600,400,1)->save($newPath);
        $image->open($newPath);
        $newHeight = $image->height();

        $info = getimagesize($picture);  //获取图片文件的属性信息
        $width = $info[0]; //原图片的宽度
        ...
        if ($sizeproportion == 1) {
            ...
            if($width != 200){echo 'no';exit;}
            if($height != 200){echo 'no';exit;}
//            $h = intval(floatval(sprintf("%.1f", floatval(150 / $width))) * $height);
        ...
        switch ($info[2]) {
            case 1: //gif格式
                $srcim = imageCreateFromgif($picture);
                break;
            case 2: //jpeg格式
                $srcim = imageCreateFromjpeg($picture);
                break;
            case 3: //png格式
                $srcim = imageCreateFrompng($picture);
                break;
            case 6: //bmp格式
                $srcim = imageCreateFrompng($picture);
                break;
...
//        $y = $y * $old_y;
        ...
        //将图片裁剪为指定比例并保存
        $image->crop($width, $height,$x,$y)->save($path . '/' . $prix . $picname);
